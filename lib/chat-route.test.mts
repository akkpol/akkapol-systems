...
function request(body: unknown, headers: HeadersInit = {}) {
  return new Request("https://portfolio.test/api/chat", {
    method: "POST",
    headers: { host: "portfolio.test", origin: "https://portfolio.test", ...headers },
...
function setup(overrides: Partial<ChatRouteDependencies> = {}) {
  const calls = {
    stream: [] as unknown[][],
    limits: [] as Array<[string, number]>,
    transcripts: [] as ChatTranscript[],
    leads: [] as ChatLead[],
  };
  ...
  const dependencies: ChatRouteDependencies = {
    ...
    consumeDailyLimit: async (key, limit) => {
      calls.limits.push([key, limit]);
      return allowed(limit);
    },
    saveTranscript: async (transcript) => { calls.transcripts.push(transcript); },
    extractLead: (_messages, sessionId) => ({
...
test("POST returns 429 when the device quota is exhausted", async () => {
  const { POST, calls } = setup({
    consumeDailyLimit: async (key, limit) => {
      calls.limits.push([key, limit]);
      return { allowed: false, count: 30, remaining: 0, limit };
    },
  });
  ...
  assert.equal(calls.limits.length, 1);
  assert.equal(calls.stream.length, 0);
});
...
test("POST returns 429 when the global quota is exhausted", async () => {
  ...
  const { POST, calls } = setup({
    consumeDailyLimit: async (key, limit) => {
      ...
      return invocation === 1
        ? allowed(limit)
        : { allowed: false, count: 500, remaining: 0, limit };
    },
  });
  const response = await POST(request({ messages: [message] }));

  assert.equal(response.status, 429);
  assert.equal(calls.limits[1]?.[0], "_total");
  assert.equal(calls.stream.length, 0);
});
...
test("POST invokes the AI provider after validation", async () => {
  ...
  assert.equal(await response.text(), "mock AI response");
  assert.deepEqual(calls.stream[0], [message]);
});
...
test("a persistence failure does not fail the streamed chat response", async () => {
  ...
  try {
    const { POST, calls } = setup({
      saveTranscript: async () => { throw new Error("Blob unavailable"); },
    });
    const response = await POST(request({ messages: [message] }));
...
test("POST saves the transcript and structured lead after the stream finishes", async () => {
  ...
  const { POST, calls } = setup({
    streamChat: async (_messages, onFinish) => {
      finish = onFinish;
      return new Response("deferred stream");
    },
  });
  const response = await POST(request({ messages: [message], id: "session_1" }));

  assert.equal(calls.transcripts.length, 0);
  assert.equal(calls.leads.length, 0);
  assert.equal(await response.text(), "deferred stream");
  await finish?.({ finishReason: "stop", usage: { totalTokens: 23 } });

  assert.deepEqual(calls.transcripts[0], {
    sessionId: "session_1",
  ...
  assert.equal(calls.leads[0]?.sessionId, "session_1");
});
